...
class SimVP_Model(nn.Module):
    ...
    def __init__(self, in_shape, t_out = None, hid_S=16, hid_T=256, N_S=4, N_T=4, model_type='gSTA',
                 mlp_ratio=8., drop=0.0, drop_path=0.0, spatio_kernel_enc=3,
                 spatio_kernel_dec=3, act_inplace=True, **kwargs):
        super(SimVP_Model, self).__init__()
        t_in, C, H, W = in_shape  # T is pre_seq_length
        self.t_in = t_in
        ...
        if model_type == 'incepu':
            self.hid = MidIncepNet(t_in*hid_S, hid_T, N_T,t_in = self.t_in, t_out = self.t_out)
        else:
            self.hid = MidMetaNet(t_in*hid_S, hid_T, N_T,
                input_resolution=(H, W), model_type=model_type,
                mlp_ratio=mlp_ratio, drop=drop, drop_path=drop_path, t_in = self.t_in, t_out = self.t_out)
    ...
    def forward(self, x_raw, **kwargs):
        ...
        skip1_bt = skip.view(B, T, C_, skip.shape[-2], skip.shape[-1])  # [B, t_in, C', H1, W1]
        skip_out = skip1_bt[:, -self.t_out:]       
        skip1_out = skip_out.reshape(B*self.t_out, C_, skip.shape[-2], skip.shape[-1])

        Y = self.dec(hid, skip1_out)
...
class MidIncepNet(nn.Module):
    ...
    def __init__(self, channel_in, channel_hid, N2, incep_ker=[3,5,7,11], groups=8, t_in=None, t_out=None, **kwargs):
        super(MidIncepNet, self).__init__()
        assert N2 >= 2 and len(incep_ker) > 1
        self.N2 = N2
        self.hid = channel_in // t_in
        self.t_out = t_out
        channel_out = self.t_out * self.hid
    ...
    def forward(self, x):
        ...
        y = z.reshape(B, self.t_out, self.hid, H, W)
        return y
...
class MidMetaNet(nn.Module):
    ...
    def __init__(self, channel_in, channel_hid, N2,
                 ...
                 mlp_ratio=4., drop=0.0, drop_path=0.1, t_in = None, t_out = None):
    ...
    def forward(self, x):
        ...
        H2, W2 = z.shape[-2], z.shape[-1]
        y = z.reshape(B, self.t_out, self.hid, H2, W2)
        return y
