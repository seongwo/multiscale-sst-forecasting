...
class TimeSformer(nn.Module):
    ...
    def __init__(
        ...
        # one-shot head: per patch representation -> Tout tokens
    ...
    def forward(self, x):
        ...
        # choose representation per patch (last time step)
        rep = h[:, -1, :, :]  # (B, N, D)

        # one-shot forecast tokens per patch
        fut = self.future_head(rep).view(B, N, self.tout, self.d_model).permute(0, 2, 1, 3)  # (B, Tout, N, D)

        # decode to frames
        y = self._unpatchify(fut, meta)  # (B, Tout, C, H, W)
        return y
