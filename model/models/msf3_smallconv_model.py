...
class SmallScaleRefiner(nn.Module):
    """Very small per-scale spatial refiner applied after token_to_patch decoding.

    Two modes are supported:
      - residual: y_refined = y + conv(y)
      - replace : y_refined = conv(y)

    The refiner operates frame-wise on (B*Tout, C, H, W).
    """

    def __init__(self, channels: int, hidden_channels: int = None, mode: str = "residual"):
        super().__init__()
        hidden_channels = int(hidden_channels or channels)
        self.mode = str(mode)
        if self.mode not in {"residual", "replace"}:
            raise ValueError(f"Unsupported decoder refine mode: {self.mode}")
...
class MSFormerV3SmallConv(nn.Module):
    def __init__(
        self,
        image_size=32,
        in_chans=1,
...
class MSFv3SmallConv(nn.Module):
    def __init__(self, **kwargs):
        super().__init__()
        kwargs.setdefault("fusion", "sum")
        self.net = MSFormerV3SmallConv(**kwargs)

    def forward(self, x):
        return self.net(x)


class MSFGv3SmallConv(nn.Module):
    def __init__(self, **kwargs):
        super().__init__()
        kwargs["fusion"] = "gated"
        self.net = MSFormerV3SmallConv(**kwargs)
...
class MSFG2v3SmallConv(nn.Module):
    def __init__(self, **kwargs):
        super().__init__()
