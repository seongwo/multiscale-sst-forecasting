...
class MSFormerV3(nn.Module):
    def __init__(
        ...
        self.use_scale_emb = bool(use_scale_emb)
        self.use_pos_emb = bool(use_pos_emb)

        self.freeze_time_emb = bool(freeze_time_emb)
        self.freeze_scale_emb = bool(freeze_scale_emb)
        self.freeze_pos_emb = bool(freeze_pos_emb)
        ...
        # checkpoint 호환성을 위해 항상 생성하고, 실제 사용은 use_*_emb로 제어
        ...
        if self.freeze_time_emb:
            self.time_emb.weight.requires_grad_(False)
        if self.freeze_scale_emb:
            self.scale_emb.weight.requires_grad_(False)
        if self.use_pos_emb and self.freeze_pos_emb:
            for s in self.scales:
                self.pos_emb[str(s)].weight.requires_grad_(False)
    ...
    def forward(self, x):
        ...
        if self.use_time_emb:
            time_term = self.time_emb(t_idx).view(1, Tin, 1, self.d_model)
