...
from model.models.predrnnv2_model import PredRNNv2_Model
...
def main():
    cfg = get_default_cfg()
    cfg["model_return_aux_loss"] = True
    cfg["aux_coef"] = float(cfg.get("predrnnv2_aux_coef", 1.0))
    set_seed(cfg["seed"])

    exp_name = cfg.get("exp_name", "predrnnv2")
    ...
    cfg_model = dict(cfg)
    cfg_model["in_shape"] = (cfg["input_len"], 1, H, W)

    if "num_layers" not in cfg_model:
        cfg_model["num_layers"] = len(cfg_model["num_hidden"])

    model = PredRNNv2_Model(
        num_layers=int(cfg_model["num_layers"]),
        num_hidden=list(cfg_model["num_hidden"]),
        configs=cfg_model,
    ).to(cfg["device"])
