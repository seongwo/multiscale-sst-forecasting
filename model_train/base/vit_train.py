...
from model.models.vit_model import TorchVisionViT
...
def main():
    ...
    # ViT hyperparams (you can edit in configs.py)
    cfg.setdefault("image_size", 32)
    cfg.setdefault("patch_size", 4)
    cfg.setdefault("embed_dim", 128)
    cfg.setdefault("depth", 4)
    cfg.setdefault("num_heads", 4)
    cfg.setdefault("mlp_ratio", 4.0)
    cfg.setdefault("vit_dropout", 0.0)
    cfg.setdefault("vit_attn_dropout", 0.0)
    cfg.setdefault("vit_freeze", False)
    cfg.setdefault("rollout_mode", "ar")  # "ar" or "one_shot"
    ...
    # --- load mask & datasets (your build_datasets requires mask)
    mask = np.load(cfg["mask_path"]).astype(bool)
    H, W = mask.shape

    # Safety: if image_size not matching actual H, override to H (ViT wants fixed image_size)
    ...
    # --- train with your project's trainer signature
    ...
    base = {
        ...
        "vit_attn_dropout": cfg["vit_attn_dropout"],
